...
def run(name: str, title: str):
    ...
    if analyze:
        ...
        if not text:
            st.warning("Pastikan teks diisi.")
        else:
            ...
            chart = alt.Chart(df).mark_bar(height=30, tooltip=None).encode(
            ...
            # Category labels underneath, aligned by proportional columns
            # c1, c2 = st.columns([sarcastic_pct, nonsarcastic_pct])
            # with c1:
            #     st.write("Sarcastic")
            # with c2:
